...
class CourseEdit extends Component {
  ...
  onContentDelete = (course_id, section_id, content_id) => {
            ...
            <Col md="8" id="course-sections" style={{ height: "740px", overflowY: "auto"}} >
              ...
              </div>
              {
                course && course.sections ?
                  course.sections.map(
                    (item, key) => <CourseSection sectionLength={course.sections.length} content={content} courseID={course.pk} section={item} key={item.id} /> 
                  )
                  :
                  ""
              }
              {
                course ?
                  <Link to={`/courses/me/manage/course/${course.pk}/sections/add/`} className="btn btn-outline-secondary mb-24pt mb-sm-0">
                    Add Section
                  </Link> :
                  ""
              }
                    ...
                    <Input onChange={this.handleChange} type="select" name="category" className="custom-select" value={this.state.category}>
                      {
                        categories ?
                          categories.map(
                            (item, key) => <option value={item.name.toLowerCase()} key={key}>{item.name}</option>) :
...
export default withRouter(connect(
  mapStateToProps,
  { getCourseDetail, getCourseSectionContent, getAllCategories, updateCourse, deleteContent }
)(CourseEdit));